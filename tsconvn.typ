...
#import "@preview/physica:0.9.8": dv, dd, expval
...
二スケール収束は@r_n89 で導入され、@r_a92 で理論が整備された概念である。
本ノートでは解説論文@r_lnw02, @r_a12 に基づいて解説する。
...
= integral_Omega expval(f(x, y))_y dd(x)
= integral_Omega 1/abs(square) integral_square f(x, y) dd(y) dd(x)
$ <e_tsconv_p>
を示すことである。
ここで$expval(f(x, y))_y = 1/abs(square) integral_square f(x, y) dd(y)$は$f(x, y)$の$y$変数に関する平均を表す。
...
= integral_Omega phi(x)expval(g) dd(x)
$ <e_tsconv_p_sep>
とでき、$ZZ^N$-周期的な関数$g$に対して$L^oo (RR^N)$関数の列$g(x/epsilon)$が$epsilon -> 0$で平均$expval(g)$に汎弱収束することを意味する。
この式も近似することを考えると$phi$,
...
ここで、
$
&abs(integral_Omega f(x, x/epsilon) dd(x)-integral_Omega expval(f(x, y))_y dd(x)) \
&quad <= integral_Omega abs(f(x, x/epsilon)-f_n (x, x/epsilon)) dd(x)+integral_Omega abs(f_n (x, x/epsilon)-expval(f_n (x, y))_y) dd(x)+integral_Omega abs(expval(f_n (x, y))_y-expval(f (x, y))_y) dd(x) \
&quad <= 2 norm(f_n-f)_(L^1(Omega; C(TT^N)))+integral_Omega abs(f_n (x, x/epsilon)-expval(f_n (x, y))_y) dd(x)
$
...
-> integral_Omega expval(u(x, y)phi(x, y))_y dd(x)
...
-> integral_Omega expval(u(x, y)^2)_y dd(x)
$
...
$
&abs(integral_Omega u_epsilon (x)phi(x, x/epsilon) dd(x)-integral_Omega expval(u(x, y)phi(x, y))_y dd(x)) \
&quad <= integral_Omega abs(u_epsilon (x)phi(x, x/epsilon)-macron(u)(x)phi(x, x/epsilon)) dd(x)+integral_Omega abs(macron(u)(x)phi(x, x/epsilon)-expval(macron(u)(x)phi(x, y))_y) dd(x) \
&quad <= norm(u_epsilon-u)_(L^2(Omega))norm(phi(x, x/epsilon))_(L^2(Omega))+integral_Omega abs(macron(u)(x)phi(x, x/epsilon)-expval(macron(u)(x)phi(x, y))_y) dd(x)
$
...
#proposition[
$L^2(Omega)$の関数列$u_epsilon (x)$が$u(x, y) in L^2(Omega times TT^N)$に二スケール弱収束するならば、$u_epsilon (x)$は$macron(u)(x) = expval(u(x, y))_y$に弱収束する。
]
...
-> integral_Omega expval(u(x, y)phi(x))_y dd(x)
...
-> integral_Omega expval(u(x, y)v(x, y))_y dd(x)
$
が成り立つ。
]
...
&-> integral_Omega expval(v(x, y)^2-2 v(x, y)v(x, y)+v(x, y)^2)_y dd(x)
= 0
$
である。
ここで、
$
&abs(integral_Omega u_epsilon (x)v_epsilon (x) dd(x)-integral_Omega expval(u(x, y)v(x, y))_y dd(x)) \
&quad <= integral_Omega abs(u_epsilon (x)v_epsilon (x)-u_epsilon (x)v(x, x/epsilon)) dd(x)+integral_Omega abs(u_epsilon (x)v(x, x/epsilon)-expval(u(x, y)v(x, y))_y) dd(x) \
&quad <= norm(u_epsilon)_(L^2(Omega))norm(v_epsilon (x)-v(x, x/epsilon))_(L^2(Omega))+integral_Omega abs(u_epsilon (x)v(x, x/epsilon)-expval(u(x, y)v(x, y))_y) dd(x)
$
...
$
integral_Omega u_epsilon (x)phi(x, x/epsilon) dd(x)
-> integral_Omega expval(tilde(u)(x, y)phi(x, y))_y dd(x)
quad forall phi in L^2(Omega; C(TT^N)),
$
$
integral_Omega gradient u_epsilon (x) dot Phi(x, x/epsilon) dd(x)
-> integral_Omega expval(p(x, y)Phi(x, y))_y dd(x)
...
$
-integral_Omega expval(tilde(u)(x, y)div_y Phi(x, x/epsilon))_y dd(x) = 0
$
がわかる。
...
$
integral_Omega expval(p(x, y)Phi(x, y))_y dd(x)
= -integral_Omega expval(tilde(u)(x, y)div_x Phi(x, x/epsilon))_y dd(x)
$
もわかり、部分積分を戻すと
$
-integral_Omega expval(tilde(u)(x, y)div_x Phi(x, x/epsilon))_y dd(x)
= -integral_Omega expval(u(x)div_x Phi(x, x/epsilon))_y dd(x)
= integral_Omega expval(gradient u(x) dot Phi(x, x/epsilon))_y dd(x)
$
...
$
cases(
	-div_x (expval(A(x, y)(gradient u(x)+gradient_y v(x, y)))_y) = f(x) "in" Omega"," ,
...
で弱形式
$
integral_Omega expval(A(x, y)(gradient u(x)+gradient_y v(x, y)) dot (gradient phi(x)+gradient_y psi(x, y)))_y dd(x)
= integral_Omega f(x)phi(x) dd(x) &\
...
&quad <= integral_Omega f(x)u(x) dd(x)-integral_Omega expval(A(x, y)(gradient u(x)+gradient_y v(x, y)) dot (gradient u(x)+gradient_y v(x, y)))_y dd(x)
$
...
$
integral_Omega expval(A(y)(gradient u(x)+gradient_y v(x, y)) dot (gradient phi(x)+gradient_y psi(x, y)))_y dd(x)
= integral_Omega f(x)phi(x) dd(x) &\
forall phi in H^1_0(Omega), psi in L^2(Omega; H^1(TT^N)) &
$
である。
...
$
integral_Omega expval(A(y)(xi+gradient_y v_xi (x, y)) dot (gradient phi(x)+gradient_y psi(x, y)))_y dd(x)
= integral_Omega f(x)phi(x) dd(x) &\
forall phi in H^1_0(Omega), psi in L^2(Omega; H^1(TT^N)) &
$
で、特に$phi = 0$を選ぶことで
$
integral_Omega expval(A(y)(xi+gradient_y v_xi (x, y)) dot gradient_y psi(x, y))_y dd(x) = 0
quad forall psi in L^2(Omega; H^1(TT^N))
$
が成り立つ。
ここから$v_xi$は$x$によらないことがわかり、$v_xi (x, y) = v_xi (y)$とおくことができ、_セル問題_
$
expval(A(y)(xi+gradient_y v_xi (y)) dot gradient_y psi(y))_y = 0
...
$
expval(A(y)(gradient u(x)+sum_(i=1)^N u_(x_i) (x) gradient_y v_i (y)) dot gradient_y psi(y))_y = 0
quad forall psi in H^1(TT^N)
$
...
$
integral_Omega expval(A(y)(gradient u(x)+sum_(i=1)^N u_(x_i) (x) gradient_y v_i (y)) dot gradient phi(x))_y dd(x)
= integral_Omega f(x)phi(x) dd(x)
quad forall phi in H^1_0(Omega).
$
$u$が満たす方程式の拡散係数$macron(A)$を考えると、
$
expval(A(y)(gradient u(x)+sum_(i=1)^N u_(x_i) (x) gradient_y v_i (y)))_y
&= expval(A(y))_y gradient u(x)+sum_(i=1)^N expval(A(y) gradient_y v_i (y))_y u_(x_i) (x) \
&= expval(A(y)+sum_(i=1)^N A(y) gradient_y v_i (y) times.o bold(e)_i)_y gradient u(x)
$
より、
$
macron(A) = expval(A(y)+sum_(i=1)^N A(y) gradient_y v_i (y) times.o bold(e)_i)_y.
$
ここから
$
macron(A)_(i j)
= expval(A_(i j) (y)+A(y) gradient v_(j) (y) dot bold(e)_i)_y
= expval(A(y) (bold(e)_j+gradient v_(j) (y)) dot bold(e)_i)_y
$
で、さらに$xi = bold(e)_j$のセル問題で$psi = v_i$とした時の式を使うと
$
macron(A)_(i j)
= expval(A(y) (bold(e)_j+gradient v_(j) (y)) dot (bold(e)_i+gradient v_(i) (y)))_y
$
を得る。
...
この時、セル問題は
$
expval(a(y)(1+v_y (y)) psi_y (y))_y = 0
...
$
macron(A)xi dot xi = expval(A(y)(xi+gradient_y v_xi (y)) dot (xi+gradient_y v_xi (y)))_y
quad xi in RR^N.
$
ただし、$v_xi$はセル問題の解である。
ここで、$v_xi$はエネルギー
$
E_xi [v] = 1/2 expval(A(y)(xi+gradient_y v(y)) dot (xi+gradient_y v(y)))_y
quad v in H^1(TT^N)
$
の最小限としても特徴づけられることに注意する。
そのため、$macron(A)$は次の変分公式を満たす。
$
macron(A)xi dot xi = min_(v in H^1(TT^N)) expval(A(y)(xi+gradient_y v(y)) dot (xi+gradient_y v(y)))_y.
$

ここから$v equiv 0$を考えることで、直ちに上からの評価
$
macron(A)xi dot xi <= expval(A(y)xi dot xi)_y
$
を得る。
...
$
macron(A)xi dot xi = min_(p in L^2(TT^N)^N, expval(p) = 0) expval(A(y)(xi+p(y)) dot (xi+p(y)))_y.
$
...
よって、
$
lambda = 2 expval(A(y)^(-1))_y^(-1) xi
$
となり、
$
macron(A)xi dot xi
>= 1/4 expval(lambda dot A(y)^(-1) lambda)_y
= expval(expval(A(y)^(-1))_y^(-1) xi dot A(y)^(-1) expval(A(y)^(-1))_y^(-1) xi)_y
= expval(A(y)^(-1))_y^(-1) xi dot xi
$
を得る。

以上をまとめると均質化拡散係数$macron(A)$は
$
expval(A(y)^(-1))_y^(-1) <= macron(A) <= expval(A(y))_y
$
を満たす。
...
実際、$R > 0$に対して
$
&abs(integral_0^oo f(x, n x) dd(x)-integral_0^oo expval(f(x, y))_y dd(x)) \
&quad <= abs((integral_0^oo-integral_0^R) f(x, n x) dd(x))+integral_0^R abs(f(x, n x)-expval(f(x, y))_y) dd(x)+abs((integral_0^R-integral_0^oo) expval(f(x, y))_y dd(x)) \
&quad <= 2 integral_R^oo sup_(y) abs(f(x, y)) dd(x)+integral_0^R abs(f(x, n x)-expval(f(x, y))_y) dd(x)
$
と評価できるためである。
]
